...
const RevenueChart = memo(({ data, type = 'line', height = 300 }) => {
  ...
  const chartHeight = useMemo(() => `${height}px`, [height]);
      ...
      return (
        <div className="bg-popover border border-border rounded-lg p-3 shadow-pronounced">
          ...
          {payload?.map((entry, index) => (
            <div key={`${entry?.dataKey}-${index}`} className="flex items-center space-x-2 text-sm">
              <div 
    ...
    return (
      <div className="w-full" style={{ height: chartHeight }}>
        <ResponsiveContainer width="100%" height="100%">
          <AreaChart data={mockData} margin={chartMargin}>
            <defs>
  ...
  return (
    <div className="w-full" style={{ height: chartHeight }}>
      <ResponsiveContainer width="100%" height="100%">
        <LineChart data={mockData} margin={chartMargin}>
          <CartesianGrid strokeDasharray="3 3" stroke="rgb(229, 231, 235)" />
          <XAxis 
            dataKey="month" 
            stroke="rgb(107, 114, 128)"
