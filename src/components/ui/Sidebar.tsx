      ...
      `}>
        <div className="flex flex-col h-full">
          ...
          <nav className="flex-1 overflow-y-auto overflow-x-hidden py-4">
            {/* CORE Section */}
            {filteredNavItems.filter(item => coreNavItems.some(nav => nav.href === item.href)).length > 0 && (
              <div className="px-3 mb-6">
                {!isCollapsed && (
                  <div className="px-3 mb-2">
                    <p className="text-xs font-semibold text-muted-foreground/70 uppercase tracking-wider">CORE</p>
                  </div>
                )}
                {filteredNavItems.filter(item => coreNavItems.some(nav => nav.href === item.href)).map((item) => (
                  <NavLink
                    key={item.name}
                    to={item.href}
                    className={({ isActive }) => `
            ...
            {/* OPERATIONS Section */}
            {filteredNavItems.filter(item => operationsNavItems.some(nav => nav.href === item.href)).length > 0 && (
              <div className="px-3 mb-6">
                {!isCollapsed && (
                  <div className="px-3 mb-2">
                    <p className="text-xs font-semibold text-muted-foreground/70 uppercase tracking-wider">OPERATIONS</p>
                  </div>
                )}
                {filteredNavItems.filter(item => operationsNavItems.some(nav => nav.href === item.href)).map((item) => (
                  <NavLink
                    key={item.name}
                    to={item.href}
                    className={({ isActive }) => `
            ...
            {/* BILLING Section */}
            {filteredNavItems.filter(item => billingNavItems.some(nav => nav.href === item.href)).length > 0 && (
              <div className="px-3 mb-6">
                {!isCollapsed && (
                  <div className="px-3 mb-2">
                    <p className="text-xs font-semibold text-muted-foreground/70 uppercase tracking-wider">BILLING</p>
                  </div>
                )}
                {filteredNavItems.filter(item => billingNavItems.some(nav => nav.href === item.href)).map((item) => (
                  <NavLink
                    key={item.name}
                    to={item.href}
                    className={({ isActive }) => `
