...
import { RentalOrderInsertPayload, RentalOrderItemInsertPayload, InvoiceInsertPayload } from '@/types/database';
...
export class OrderService {
  ...
  static async createOrder(orderData: OrderFormData & { outlet_id?: string }): Promise<Order> {
    try {
      ...
      const insertData: RentalOrderInsertPayload = {
        // order_number will be auto-generated by trigger
        customer_id: orderData.customer_id,
        event_date: orderData.event_date,
        delivery_date: (orderData as OrderFormData & { delivery_date?: string }).delivery_date || orderData.event_date,
        return_date: (orderData as OrderFormData & { return_date?: string }).return_date || orderData.event_date,
        ...
        security_deposit: (orderData as OrderFormData & { security_deposit?: number }).security_deposit || 0,
      ...
      // Create order items
      if (orderData.items.length > 0) {
        const orderItems: RentalOrderItemInsertPayload[] = orderData.items.map(item => ({
          order_id: data.id,
        ...
        const attemptCreateInvoice = async (_attemptNumber: number): Promise<any> => {
          ...
          const invoiceDate: string = new Date().toISOString().split('T')[0]!; // Today's date
          ...
          const dueDate: string = (dueDateObj > defaultDueDate 
            ? dueDateObj.toISOString().split('T')[0]
            : defaultDueDate.toISOString().split('T')[0])!;

          // Prepare invoice payload
          const invoicePayload: InvoiceInsertPayload = {
          ...
          const invoiceItemsToInsert = taxResult.breakdown.map((line, index) => {
            ...
            const rentalDays = orderItem?.rental_days || 1;
        ...
        if (!createdInvoice) {
          ...
          throw new Error(`Invoice creation failed after ${MAX_INVOICE_CREATION_ATTEMPTS} attempts: ${errorMessage}`);
        }
  ...
  static async recreateInvoiceForOrder(orderId: string): Promise<Invoice> {
    try {
      ...
      if (!createdInvoice) {
        const errorMessage = lastErr instanceof Error ? lastErr.message : String(lastErr);
        throw new Error(`Invoice recreation failed after ${MAX_INVOICE_CREATION_ATTEMPTS} attempts: ${errorMessage}`);
      }
